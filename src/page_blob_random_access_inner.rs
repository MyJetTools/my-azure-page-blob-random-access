...
pub struct PageBlobRandomAccessInner {
...
impl PageBlobRandomAccessInner {
    pub fn new(
    ...
    pub async fn create_new(&mut self, pages: usize) -> Result<usize, AzureStorageError> {
        let result = self.page_blob.create_if_not_exists(pages).await?;
        self.page_blob_size = Some(result);
        Ok(result)
    }
}
...
mod test {
    ...
    async fn test_basic_cases() {
        ...
        let mut random_access = PageBlobRandomAccessInner::new(page_blob, true, 10);
